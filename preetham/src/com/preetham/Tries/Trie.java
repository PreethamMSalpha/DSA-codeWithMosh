...
//        public static int ALPHABET_SIZE = 26;
    ...
    private void traverse(Node root) {
        //note => preorder for insertion and postorder for deletion
    ...
    private void remove(Node root, String word, int index) {
        ...
        if (!child.hasChildren() && !child.isEndOfWord) {
            root.removeChild(ch); //check line 104
        }

//        System.out.println(root.value); //here 1st is recursion and then printing value which is post-order
    }
    ...
    public static String longestCommonPrefix(String[] words) {
        ...
        while (prefix.length() < maxChars) {
            ...
            if (children.length != 1) { //if there are deviation
                break;
            }
    ...
    private void printChildren(Node root) {
        for (var child: root.getChildren())
